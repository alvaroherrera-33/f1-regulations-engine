...
# The model + tokenizer are VENDORED in the repo (backend/models/) so the worker
# never downloads from HuggingFace at runtime — that runtime download was the
# root cause of cold-start 502s on the free tier (HF rate-limits + memory spike).
# Quantized int8 export (~23MB): same vector space as the stored embeddings
# (verified: retrieves the correct articles from the existing DB).
_MODELS_DIR = Path(__file__).resolve().parent.parent / "models"
_LOCAL_MODEL = _MODELS_DIR / "model_quantized.onnx"
_LOCAL_TOKENIZER = _MODELS_DIR / "tokenizer.json"
...
class LocalEmbeddingsGenerator:
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2"):
        logger.info("Loading vendored ONNX embedding model from %s", _LOCAL_MODEL)
        if not _LOCAL_MODEL.exists() or not _LOCAL_TOKENIZER.exists():
            raise RuntimeError(
                f"Vendored model not found at {_MODELS_DIR}. Expected "
                "model_quantized.onnx + tokenizer.json committed in the repo."
            )
        model_path = str(_LOCAL_MODEL)
        tokenizer_path = str(_LOCAL_TOKENIZER)
